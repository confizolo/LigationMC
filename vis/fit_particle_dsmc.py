...
def get_md_target_pmf(nlin=64):
    md_csv = "/storage/cmstore02/groups/TAPLab/fconforto-projects/fconforto-olympic-gels/results/histories/dist_cyclized_linear_length_by_nlin_all_systems.csv"
...
def main():
    ...
    nlin = 64
    mlin = 6
    n_trials = 20000  # High number of trials for smooth objective function
    
    # We want to optimize k2. Since the particle DSMC generates smaller rings, we likely need to decrease k2 
    # to reduce the cyclisation rate relative to merging. 
    # The original k2 for Gillespie SSA is 12840.85. Let's search around there.
    
    def objective(k2):
        if k2 <= 0:
            return 1e6
        rng = np.random.default_rng(42)  # Fixed seed for reproducibility and smooth objective
        sim_pmf = simulate_particle_length_pmf(k1, k2, nlin, mlin, nu, n_trials, rng)
        js = _js_divergence(sim_pmf, md_pmf)
        print(f"k2 = {k2:.2f} -> JS = {js:.6e}")
        return js

    print("\nStarting optimization...")
    # Bracket the minimum around the Gillespie SSA optimal k2
    res = minimize_scalar(objective, bounds=(5000, 15000), method='bounded', options={'xatol': 1.0, 'maxiter': 50})
    ...
    out_path = "./results/compare/fitted_k1_k2_particle.json"
